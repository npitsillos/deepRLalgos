...
"""
Notes:
    - This version trains without using the model. It's pretty much
    reinforce_lambda_truncated2.py and gets the similar results when using the
    same network parameters
    - However, its alot more unstable using 3 layers of 64
    - For DRL, normal network architectures are [64,64], [256,256] and [400,400]
        - I have seen [64,64,64] but its less common
        - I have seen other papers propose larger network architectures BUT they
        also proposed some sort of alteration to the network architecture to
        make it more applicable
    - The learning rate is very high which may also contribute to instabilities
"""
...
logger = Logger2("dreamer_noModel_medActorCriticNormLr_5Step")
...
for epoch in range(1, 2001):
    ...
    paths = replay_buffer.random_batch(200, 4)
