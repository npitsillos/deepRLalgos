...
NOTES:
    ...
    the lambda version, but with a reduced policy learning rate they seem to be
    equivalent.
        - I believe the intuition behind having a slower learning rate for the
        policy is so that the critic has an easier time keeping up
"""
...
for epoch in range(4000):
    ...
    for path in paths:
        ...
        returns = lambda_returns(
            path["rewards"], path["observations"], final_state, path["terminals"], critic_target, 1)
...
plt.savefig("reinforce_nolambda_results.png")
plt.clf()
plt.plot(range(10, (len(policy_losses)+1)*10, 10), policy_losses)
plt.savefig("reinforce_nolambda_policy.png")
plt.clf()
plt.plot(range(10, (len(value_losses)+1)*10, 10), value_losses)
plt.savefig("reinforce_nolambda_values.png")
