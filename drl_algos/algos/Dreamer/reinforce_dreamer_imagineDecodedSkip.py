...
import drl_algos
from drl_algos.networks.myrssm import RSSMDiscState
...
logger = Logger2("dreamer_medModel_medActorCriticFastLR_imagineDecodedSkip5")
...
for epoch in range(1, 2001):
    ...
    posteriors = RSSMDiscState(
        posteriors.logit[5:], posteriors.stoch[5:], posteriors.deter[5:]
    )
    discounts = discounts[5:]

    states, actions, rewards, discounts, weights = model.imagine(
        actor_critic.policy, posteriors, discounts, 10, actor_critic.discount
    )
