...
"""
Notes:
    - With Medium Model and Fast Actor Critic is converges nicely to a stable
    solution
    - it is a bit less stable until the discount model gets a bit more accurate
    althought this does not take alot longer than learning without the model
    - Accurate terminal prediction is clearly important for success of the
    actor critic and cannot be compensated with by using lower learning rate
    or hard constraints on the discount
"""
...
logger = Logger2("dreamer_medModel_medActorCriticNormLR_onPolicy_discount")
...
for epoch in range(1, 2001):
    ...
    # reward_loc = model.reward_model(states)
    discount_logits = model.discount_model(states)
    ...
    discounts = discount_dist.mean * actor_critic.discount
    weights = torch.cumprod(
        torch.cat((torch.ones_like(discounts[0:1]), discounts[1:])), 0
    ).to(actor_critic.device)
