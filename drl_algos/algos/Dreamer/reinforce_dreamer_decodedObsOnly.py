...
"""
Notes:
    - With a Small Model and Fast Actor Critic it converges to a pretty stable
    solution although it does wobble a bit near the end of training
        - Evaluation is alot more stable than exploration
    - Potentially, it would perform better if the PathCollectors used the
    decoded observation instead of the true observations
    - It may be that with low state observations it is better to decode them
    rather than train in the latent state
        - Not ideal since lose out on recurrency and features but it may work
        with imagination
"""
...
logger = Logger2("dreamer_medModel_medActorCriticFastLR_onPolicy_decodedObs")
