...
"""
    ...
    issues is probably due to the model
        - coded this while pretty tired so will need to be double checked
        - seems to show signs of learning but can easily go astray
        - haven't included gradient clipping for actor-critic, same network
        architecture etc which is all likely to have an effect
        - I should integrate the logger and start viewing what the model is
        doing

    - need to add evaluation, which also means making the model deterministic
...
def lambda_returns(rewards, gammas, values, lam=0.95, discount=0.99):
    ...
    for i in range(len(rewards[0])-2, -1, -1):
        ...
        if prev_target is None:
            target = gamma * (reward + next_gamma * next_value)
        else:
            target = gamma * (reward + next_gamma * ((1.0-lam)*next_value + lam*prev_target))
...
logger = Logger2("dreamer-reinforce2")
