...
class RolloutBuffer():
    ...
    def __init__(self, max_replay_buffer_size, env, is_recurrent = False, env_info_sizes = None):
        self.env = env
        ...
        if self._is_recurrent:
            self.actor_hxs = np.zeros((self.buffer_size, self.recurrent_size))
            self.actor_cxs = np.zeros((self.buffer_size, self.recurrent_size))
            self.critic_hxs = np.zeros((self.buffer_size, self.recurrent_size))
            self.critic_cxs = np.zeros((self.buffer_size, self.recurrent_size))
    ...
    def random_batch(self, batch_size):
        ...
        for key in self._env_info_keys:
            assert key not in batches.keys()
            batches[key] = self._env_infos[key][indices]
        return batches
